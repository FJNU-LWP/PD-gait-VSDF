...
class Dual_Net(nn.Module):
	...
	def __init__(self,
		...
		self.return_cls_token = return_cls_token
		############
		#self.pos_embed1 = nn.Parameter(torch.zeros(1, 10, self.embed_dims))
		#self.time_embed1 = nn.Parameter(torch.zeros(1, 9, 768))

		#pos_embed1 = pos_embed1.to('cuda:0')

		self.conv1 = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1)

		#self.conv1.bias.data = self.conv1.bias.data.float()

		self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
		# 输入形状: (36, 8, 64, 32, 32)，输出形状: (36, 8, 64, 16, 16)
		...
		self.patch_embed = PatchEmbed(
			...
			embed_dims=embed_dims,
			tube_size=tube_size,
			conv_type=conv_type)#3Dconve
		'''
		#num_patches = self.patch_embed.num_patches#196
		num_patches = 9#self.patch_embed.num_patches  # 196
		if self.attention_type == 'divided_space_time':
		...
		if use_learnable_pos_emb:#true

			#self.pos_embed = nn.Parameter(torch.zeros(1,num_patches,embed_dims))
			#self.time_embed = nn.Parameter(torch.zeros(1,num_frames,embed_dims))
			###########
			self.pos_embed = nn.Parameter(torch.zeros(1, 10, self.embed_dims))
			self.time_embed = nn.Parameter(torch.zeros(1, 9, 768))
			#####33
	...
	def prepare_tokens(self, x):
		...
		x = self.patch_embed(x)#3dconve
		# Add Position Embedding
		cls_tokens = repeat(self.cls_token, 'b ... -> (repeat b) ...', repeat=x.shape[0])#torch.Size([16, 1, 384])
		...
		if self.use_cls_token_temporal:
			...
			x = torch.cat((cls_tokens, x), dim=1)
			if self.use_learnable_pos_emb:#true
				x = x + self.pos_embed#torch.Size([1, 197, 384])
	...
	'''
	def prepare_shape(self, x):
		# Tokenize
		b, num_frames, patch, dims =x.shape
		x = x.view(-1, patch, dims)
		#b = 4  # x.shape[0]
		###################################
		# x = self.patch_embed(x)#3dconve
		...
		#x = x.to('cuda:0')
		#cls_tokens = cls_tokens.to('cuda:0')
		if self.use_cls_token_temporal:
	...
	'''
	def keypoint_patch_embed(self,x,coord):
		...
		for i in range(batch_size):
			for j in range(frame_count):
				for k in range(9):
					...
					# block = video_data[i, j, :, left:left + s, top:top + s]
	...
	def forward(self, x):
		...
		for batch_index in range(x.size(0)):
			...
			image=image_tensor
			image_tensor = (image - image.min()) / (image.max() - image.min())
		...
		breakpoint()
		'''
		#x:torch.Size([8, 16, 3, 224, 224])
		################
		#torch.Size([4, 8, 9, 3, 32, 32])
		batch,num_frames, patch,channels, height, width = x.shape
		#x = rearrange(x, 'b t p c h w -> b p t c h w')
		# print(x.shape)#torch.Size([4, 9, 3, 8, 32, 32])
		#batch_original=x.shape[0]

		#x = x.reshape(-1, num_frames, channels, height, width)


		#x = rearrange(x, 'b c t h w -> b t c h w')

		#batch_size = batch_original*patch
		...
		x = self.conv1(x)
		x = self.pool1(x)
		print(x.shape)#torch.Size([288, 64, 16, 16])
		x = torch.flatten(x, start_dim=1)
		print(x.shape)
		x = self.fc(x)
		print(x.shape)
		x = x.view(batch,num_frames, patch, self.embed_dims)
		print(x.shape)
		#breakpoint()

		#x = x.view(batch_original, 9, 8, self.embed_dims)

		#x = x.view(batch_original*8, 9, self.embed_dims)
		#pos_embed1 = nn.Parameter(torch.zeros(1, 10, self.embed_dims))
		#pos_embed1 = pos_embed1.to('cuda:0')
		x, cls_tokens, b =self.prepare_shape(x)

		#############################
		#x, cls_tokens, b = self.prepare_tokens(x)#x:torch.Size([64, 197, 768]);cls_token:torch.Size([64, 1, 768]),b:8
		if self.attention_type != 'fact_encoder':
			x = self.transformer_layers(x)
			...
			x = spatial_transformer(x)#x:torch.Size([64, 197, 768])
			# Add Time Embedding
			cls_tokens = x[:b, 0, :].unsqueeze(1)
			...
			if self.use_learnable_pos_emb:#true
				...
				#time_embed1 = nn.Parameter(torch.zeros(1, 9, 768))
				#time_embed1=time_embed1.to('cuda:0')
				print(x.shape)
				x = x + self.time_embed
...
def parse_args():
	...
	# Model
	parser.add_argument(
		'-arch', type=str, default='Dual_Net',
		help='the choosen model arch from [timesformer, Dual_Net]')
