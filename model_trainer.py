...
from keypoint import extract_keypoint
from keypoint_patch import keypoint_patch_embed
from op_keypoint import op_extract_keypoint
#################
...
from dual_modality import TimeSformer, Dual_Net, MaskFeat
...
class VSDF(pl.LightningModule):
	...
	def __init__(self, 
		...
		if self.configs.objective =='mim':
			...
			self.train_top5_acc = Accuracy(top_k=3)#(top_k=5)
		...
		if self.do_eval:
			...
			self.val_top5_acc = Accuracy(top_k=3)#(top_k=5)
		if self.do_test:
			self.n_crops = n_crops
			self.test_top1_acc = Accuracy()
			self.test_top5_acc = Accuracy(top_k=3)#(top_k=5)
	...
	def training_step(self, batch, batch_idx):
		...
		if self.configs.objective == 'mim':
			...
			#coord=extract_keypoint(inputs)#torch.Size([4, 8, 49, 2])
			coord = op_extract_keypoint(inputs)  # torch.Size([4, 8, 49, 2])
			inputs=keypoint_patch_embed(inputs,coord)
		...
		if self.do_eval:
			...
			if self.configs.eval_metrics == 'linear_prob':
				...
				if self.configs.arch == 'mvit':
					...
					##################################
					#coord = extract_keypoint(inputs)
					coord = op_extract_keypoint(inputs)
					inputs = keypoint_patch_embed(inputs, coord)
					device = torch.device("cuda:0")  # 定义 CUDA 设备
					inputs = inputs.to(device)
					print(inputs.device)
					#breakpoint()
